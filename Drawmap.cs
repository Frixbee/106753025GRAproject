...
//第一個迴圈是robot數量 第二個迴圈是多邊形數量 第三個迴圈是頂點 第二個迴圈結束後還要設定初始狀態(目標狀態)

public class Drawmap : MonoBehaviour
{
    public struct Vertice   //座標(x,y)
    {
        public float vx;
        public float vy;
    }
    public struct PolygonVertice    //多邊形頂點
    {
        public Vertice[] ver;
    }
    public struct RobotVertice  //機器人頂點
    {
        public PolygonVertice[] polyver;
    }
    public struct ObstacleVertice   //障礙物頂點
    {
        public PolygonVertice[] polyver;
    }
    ...
    public struct Config    //狀態(x,y,角度)
    {
        public float px;
        public float py;
        public float angle;
    }
    public struct PotentialPoint    //狀態(x,y,角度)
    {
        public float px;
        public float py;
        public float angle;
    ...
    public struct PotentialPointParent    //狀態(x,y,角度)
    {
        public float px;
        public float py;
        public float angle;
    }
    public struct Controll  //控制點座標
    {
        public float conpx;
        public float conpy;
    }
    public struct Apaircontroll //控制點
    {
    ...
    public void ReadRobot() //讀robot.txt檔
    {
        try
        {
            FileStream aFile = new FileStream(@"C:\Users\admin\Documents\GRAfinalproject\Assets\Resources\robot.txt", FileMode.Open);
            ...
            while (strLine != null)
            {
                if (strLine[0] != '#')  //跳過'#'的行
                {
                    robnum = Convert.ToInt32(strLine);  //robot的數量
                    ...
                    for (int i = 0; i < robnum; i++)    //依robot的數量跑迴圈
                    {
                        GameObject robot = new GameObject("robot" + i); //創造robot物件
                        GameObject Final = new GameObject("final" + i); //創造robotfinal物件
                        strLine = sr.ReadLine();
                        while (strLine[0] == '#')   //跳過'#'的行
                        {
                            strLine = sr.ReadLine();
                        }
                        ...
                        for (int j = 0; j < robpolynum; j++)   //依多邊形的數量跑迴圈
                        {
                            GameObject poly = new GameObject("poly" + j); //創造多邊形物件
                            poly.transform.parent = robot.transform;    //將多邊形物件加入機器人物件之下
                            strLine = sr.ReadLine();
                            while (strLine[0] == '#')   //跳過'#'的行
                            {
                                strLine = sr.ReadLine();
                            }
                            robvernum = Convert.ToInt32(strLine);    //頂點數量
                            robver[i].polyver[j].ver = new Vertice[robvernum];
                            for (int k = 0; k < robvernum; k++)  //依頂點數量跑迴圈
                            {
                                strLine = sr.ReadLine();
                                while (strLine[0] == '#')   //跳過'#'的行
                                {
                                ...
                                int z = 0;
                                while (strLine[z] != ' ')   //找空白的位置
                                {
                                    z++;
                                }
                                float vx = Convert.ToSingle(strLine.Substring(0, z));    //分離X座標
                                float vy = Convert.ToSingle(strLine.Substring(z + 1, strLine.Length - 1 - z));  //分離Y座標
                                robver[i].polyver[j].ver[k].vx = vx;   //加入X座標的點到第i個機器人的第j個多邊形的第k個頂點
                                robver[i].polyver[j].ver[k].vy = vy;   //加入Y座標的點到第i個機器人的第j個多邊形的第k個頂點
                            }
                            DrawRobot(i, j, robvernum, poly, robver);   //畫機器人(多邊形)
                        }
                        //讀取初始狀態
                        strLine = sr.ReadLine();
                        while (strLine[0] == '#')   //跳過'#'的行
                        ...
                        float ix = Convert.ToSingle(strLine.Substring(0, a));   //分離X座標
                        ...
                        float iy = Convert.ToSingle(temp.Substring(0, b));  //分離Y座標
                        float ia = Convert.ToSingle(temp.Substring(b + 1, temp.Length - 1 - b));    //分離角度
                        robinitial[i].px = ix; //存初始X座標值
                        robinitial[i].py = iy; //存初始Y座標值
                        robinitial[i].angle = ia;  //存初始角度值
                        //讀取目標狀態
                        strLine = sr.ReadLine();
                        while (strLine[0] == '#')   //跳過'#'的行
                        {
                        ...
                        float fx = Convert.ToSingle(strLine.Substring(0, c));   //分離X座標
                        ...
                        float fy = Convert.ToSingle(temp1.Substring(0, d));  //分離Y座標
                        float fa = Convert.ToSingle(temp1.Substring(d + 1, temp1.Length - 1 - d));   //分離角度
                        final[i].px = fx;  //存目標X座標值
                        final[i].py = fy;  //存目標Y座標值
                        final[i].angle = fa;   //存目標角度
                        //讀取controll point
                        strLine = sr.ReadLine();
                        while (strLine[0] == '#')   //跳過'#'的行
                        {
                        ...
                        int connum = Convert.ToInt32(strLine);  //控制點數量
                        ...
                        for (int k = 0; k < connum; k++)
                        {
                            ...
                            float con1 = Convert.ToSingle(strLine.Substring(0, e));                             //分離第k個控制點的X座標
                            float con2 = Convert.ToSingle(strLine.Substring(e + 1, strLine.Length - 1 - e));    //分離第k個控制點的Y座標
                        ...
                        //畫終點
                        //float fangle = final[i].angle * Mathf.Deg2Rad;
    ...
    public void ReadObstacle()  //讀obstacle.txt檔
    {
        try
        {
            FileStream aFile = new FileStream(@"C:\Users\admin\Documents\GRAfinalproject\Assets\Resources\obstacle.txt", FileMode.Open);
            StreamReader sr = new StreamReader(aFile);
            string strLine = sr.ReadLine();
            while (strLine != null)
            {
                if (strLine[0] != '#')  //跳過'#'的行
                {
                    obsnum = Convert.ToInt32(strLine);  //障礙物的數量
                    obsver = new ObstacleVertice[obsnum];
                    obsinitial = new Config[obsnum];
                    for (int i = 0; i < obsnum; i++)     //依obstacle的數量跑迴圈
                    {
                        GameObject obstacle = new GameObject("obstacle" + i);   //創建障礙物物件
                        strLine = sr.ReadLine();
                        while (strLine[0] == '#')   //跳過'#'的行
                        {
                            strLine = sr.ReadLine();
                        }
                        obspolynum = Convert.ToInt32(strLine);  //多邊形的數量
                        obsver[i].polyver = new PolygonVertice[obspolynum];
                        for (int j = 0; j < obspolynum; j++)   //依多邊形的數量跑迴圈
                        {
                            GameObject poly = new GameObject("poly");   //創建多邊形物件
                            poly.transform.parent = obstacle.transform; //將多邊形物件加入障礙物物件之下
                            strLine = sr.ReadLine();
                            while (strLine[0] == '#')   //跳過'#'的行
                            {
                                strLine = sr.ReadLine();
                            }
                            obsvernum = Convert.ToInt32(strLine);   //頂點數量
                            obsver[i].polyver[j].ver = new Vertice[obsvernum];
                            for (int k = 0; k < obsvernum; k++)  //依頂點數量跑迴圈
                            {
                                strLine = sr.ReadLine();
                                while (strLine[0] == '#')   //跳過'#'的行
                                {
                            ...
                            DrawObstacle(i, j, obsvernum, poly, obsver);    //畫多邊形
                        }
                        //讀取初始狀態
                        strLine = sr.ReadLine();
                        while (strLine[0] == '#')   //跳過'#'的行
                        ...
                        for (int p = 0; p < obspolynum; p++)    //將頂點全部更新
                        {
    ...
    public void DrawRobot(int robnum, int polynum, int vnum, GameObject obj, RobotVertice[] robver)
    {
        ...
        Material defaultMat = new Material(Shader.Find("AR/TangoARRender"));
    ...
    public void Posistion(GameObject obj, Config initial)
    {
        ...
        //obj.transform.Translate(initial.px, initial.py, 0);
    ...
    public void PotentialFeild()        //大小:128*128 254:沒拜訪過 255:障礙物 0:goal 
    {
    ...
    public void DrawPotentialFeild(float F_x,float F_y,int draw)    
    {
        ...
        Color32 firstcolor = new Color32(0, 0, 0, 0);
    ...
    public void BFS()
    {
        ...
        //List<List<PotentialPoint>> subT = new List<List<PotentialPoint>>(128);
        //List<PotentialPoint> angleT = new List<PotentialPoint>(360);
